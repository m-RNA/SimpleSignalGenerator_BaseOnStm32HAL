...
// 显示波形开启还是关闭
#define SHOW_LCD_WAVE_ON_OR_OFF LCD_DisplayChineseString(24 * 3 + 16, 319 - 24 * 6, 5 + gpSignal_Generator->State.OFF_ON * 2, 24, 2)

// 显示波形种类：正弦波/方波/三角波/锯齿波
#define SHOW_LCD_WAVE_MODE LCD_DisplayChineseString(24 * 4 + 16 + 3, 319 - 24 * 6, 14 + (gpSignal_Generator->State.Mode - 1) * 3, 24, 3)

// 显示波形实际频率
#define SHOW_LCD_WAVE_REAL_FREQ                                                                                    \
    if (gpSignal_Generator->State.Freq < 10000)                                                                    \
        snprintf((char *)LCD_String_Buffer, 20, "  Frq:%dHz            ", gpSignal_Generator->State.Freq);         \
    else                                                                                                           \
        snprintf((char *)LCD_String_Buffer, 20, "  Frq:%dkHz            ", gpSignal_Generator->State.Freq / 1000); \
