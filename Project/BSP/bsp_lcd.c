...
void LCD_DrawChinese(u16 x,u16 y,u16 index,u8 size,u8 mode)
{
	...
	{   		
        if(size == 16)
           temp=Chinese_Table_16[index][t];			//得到点阵数据      
        else if(size == 24)
           temp=Chinese_Table_24[index][t];			//得到点阵数据      
        else if(size == 32)
           temp=Chinese_Table_32[index][t];			//得到点阵数据    
        
		for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)
                LCD_DrawPoint(x,y,TextColor);
			else if(mode==0)
...
void LCD_DrawChinese16(u16 x,u16 y,u16 index)
{
  LCD_DrawChinese(x, y, index,16,0);
}

void LCD_DrawChinese24(u16 x,u16 y,u16 index)
{
  LCD_DrawChinese(x, y, index,24,0);
}

void LCD_DrawChinese32(u16 x,u16 y,u16 index)
{
  LCD_DrawChinese(x, y, index,32,0);
}

void LCD_DisplayChineseString(u16 x,u16 y,u16 index,u8 size, u16 lenth)
{
	u32 i = 0;
	u16 refcolumn = y;//319;
	...
	while ((i < lenth))	 //	20
	{
        LCD_DrawChinese(x, refcolumn, index, size,0);
